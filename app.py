...
def render_siswa():
    ...
    if active_quiz_id:
        ...
        # 6. SCRIPT JAVASCRIPT DETEKSI KECURANGAN (OPTIMAL UNTUK ANDROID)
        cheat_script = f"""
        <script>
        (function() {{
            ...
            function reportViolation() {{
                // Cek apakah elemen aktif saat ini adalah input/textarea (Keyboard Android)
                ...
                // Abaikan jika hanya pemicu keyboard / fokus input
                if (isInputActive) return;

                const now = Date.now();
                const lastReport = parseInt(sessionStorage.getItem("last_rep_" + userKey) || "0");
                
                // Cooldown 4 detik untuk mencegah trigger berantai di HP Android
            ...
            // Hanya aktifkan visibilitychange & paste (Aman dari Keyboard Android)
