...
# ==========================================
# 8. PANEL SISWA (ANTI-CHEAT FIXED - NO LOOP)
# ==========================================
def render_siswa():
    ...
    # CSS Khusus: Sembunyikan kontainer tombol trigger anti-cheat dari UI siswa
    st.markdown("""
        <style>
        .ac-hidden-trigger {
            display: none !important;
    ...
    if active_quiz_id:
        ...
        # 2. TRIGGER PELANGGARAN & JS ANTI-CHEAT STABIL
        if is_ulangan:
            # Sembunyikan tombol menggunakan wrapper class div khusus
            st.markdown('<div class="ac-hidden-trigger">', unsafe_allow_html=True)
            if st.button("TRIGGER_VIOLATION_KEY", key=f"btn_rec_viol_{tg_id}"):
                if not is_locked:
                    ...
                    if new_v >= 15:
                        ...
                        submit_jawaban_siswa(tg_sub, username_s, nama_s, kelas_s, answers, is_forced=True)
            ...
            # Injeksi JS (Hanya mendeteksi pindah tab / visibilitychange agar tidak looping)
                ...
                components.html("""
                    ...
                    (function() {
                        ...
                        function triggerViolation() {
                            const now = Date.now();
                            if (now - lastTrigger < 4000) return; // Cooldown 4 detik
                            lastTrigger = now;

                            const buttons = Array.from(parentDoc.querySelectorAll('button'));
                            const triggerBtn = buttons.find(b => b.textContent.includes('TRIGGER_VIOLATION_KEY'));
                            if (triggerBtn) {
                                triggerBtn.click();
                            }
                        }
    ...
    with tab_nilai:
        ...
        if not valid_subs:
            st.info("Belum ada riwayat nilai me untuk tugas yang aktif.")
        else:
            riwayat_rows = []
